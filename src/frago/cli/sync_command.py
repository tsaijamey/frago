"""sync 命令 - 同步 examples/ 到 resources/recipes/ 和 commands 到 resources/commands/"""
...
def sync(
    ...
    \b
    示例:
      uv run frago sync                    # 同步所有资源
      uv run frago sync --commands-only    # 仅同步 commands
      uv run frago sync --recipes-only     # 仅同步 recipes
      uv run frago sync --files "*stock*"  # 同步名称包含 stock 的资源
      uv run frago sync --dry-run          # 预览将要同步的文件
      uv run frago sync --clean            # 清理已删除的资源
    """
    try:
        # 确定同步范围
