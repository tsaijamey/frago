...
- 不要在服务端把 `FRAGO_SERVER_HOST` 留成默认的 `0.0.0.0`。部署机上设成 `127.0.0.1`，只让反代碰得到；同时写死 `FRAGO_TRUST_LAN=0`（已是默认值，写死防止环境里继承到 `=1` 把同一个 VPC 的邻居当成「本机」放行）和 `FRAGO_BEHIND_PROXY=1`（不再从对端地址推断信任，反代设不设转发头都不影响）。个人机器上 `FRAGO_SERVER_HOST` 与 `FRAGO_BEHIND_PROXY` 保持默认即可。
