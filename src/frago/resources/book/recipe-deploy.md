...
**服务器上必须显式写死这三个值：**
    ...
    FRAGO_TRUST_LAN=0              # 同网段的邻居不算「本机」；已是默认值，写死防止继承到 =1
    FRAGO_BEHIND_PROXY=1           # 干脆不从对端地址推断信任

第三条是兜底。靠「有没有反代转发头」判断请求是不是本机来的，本质是在猜别人家反代的行为，猜错就是敞开。
置位后公开区照常，其余一律要 token。
（`FRAGO_TRUST_LAN` 自 2026-09-24 起在所有机器上默认关闭，局域网来的请求要 token，个人机器想让手机直连才设 `=1`；CGNAT 段 100.64/10（Tailscale）在 Python 3.13+ 上**不算**私网，tailnet 邻居要 token。）
...
- **外部命令的放行登记**：配方写了 `uses_commands` 的，`~/.frago/recipe-data/<配方>/grants.json`
  记的是**这台机器**上那个命令装在哪、能看哪些目录，换台机器答案就不同，NEVER 拷过去。
  服务器上要备齐三样，第一次运行时它自己审计：
  - 命令本身装好、登录好，并且**不是 snap 装的**——snap 程序在配方隔离里起不来
    （2026-09-18 在 Ubuntu 上实测，报 `cannot create transient scope`），换发行版源或官方包；
  - 设置页里 CoreAgent 那一行绑了一条带密钥的连接（没绑，审计做不成，配方拒起）；
  - 部署后先以跑 frago 的那个用户手动 `frago recipe run <配方>` 一次，或 `frago schedule run <id>`：
    服务端常驻启动不做审计，没有登记就拒起。
...
**版本号不变，随包的书和 hook 规则不会更新。** frago 起服务时把包里的书、规则、CoreAgent
准则铺到 `~/.frago` 下，已有的文件只在**版本号变了**的时候才换；版本号没变只补新增的文件。
所以装的是同一个版本号下的新提交时，代码是新的，`~/.frago/book/` 里的书还是旧的
（2026-09-18 实测：服务器 1.4.57 → 1.4.57，新写进 `recipe-fields` 的内容没到）。
要让书跟上，推送前把版本号提上去。
