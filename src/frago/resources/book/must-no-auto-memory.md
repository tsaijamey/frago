...
本项目禁用 Claude Code 的 auto memory 系统。所有跨会话结构化沉淀 MUST 走 `{{frago_launcher}} def save`，所有跨会话召回 MUST 走 `{{frago_launcher}} def find`。
...
```bash
# 查看可用领域
{{frago_launcher}} def list-domains

# 沉淀到已有领域
{{frago_launcher}} <domain> save --name=<name> --content="<content>"

# 如果没有合适领域，先注册
{{frago_launcher}} def register-domain <name>
...
| 更新 MEMORY.md 索引添加新条目 | 不需要索引文件，`{{frago_launcher}} def find` 就是索引 |
