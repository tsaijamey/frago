...
**还没到点的排程也不构成不重启的理由。** 排程由 server 自己计时：它记着每条排程上次触发的时间，每一跳拿排程表算出下一个时间点，到了或过了就拉起。停机期间错过的时间点，server 起来几秒后发现已过点，会立刻补跑一次（错过几次都只补一次）。所以重启前不必看「接下来几分钟有没有排程要跑」，也不必重启后手动重跑排程。

真正会丢的只有**正在跑的那一轮**：它一开跑就算已触发，停机中途打断后不会被续上，也不会被补跑，只能等下一个时间点重新来一轮。一天只跑一次的排程（收盘、盘后筹备这类）被这样打断，就要等到明天。

```
例：某排程每天 15:02 跑一次
  15:01 停机、15:05 起来 → 15:02 那次过点未跑，起来后立刻补跑，晚 3 分钟
  15:02 已开跑、15:03 重启 → 这一轮中途丢了，今天不会再跑，等明天 15:02
```
