...
```
{{frago_launcher}} browser start        # 拉起浏览器 + 扩展桥（自动完成全链路）
{{frago_launcher}} browser <cmd> ...    # navigate / get-content / click / detect 等全部命令
{{frago_launcher}} browser stop         # 对称拆除
```
...
`{{frago_launcher}} browser start` 一条命令：选浏览器 → 拉起 native messaging daemon → 写 manifest（`<profile>/NativeMessagingHosts/`）→ 带 `--load-extension` 启动浏览器 → 等待桥握手。无需任何手工准备。
