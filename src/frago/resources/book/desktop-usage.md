...
**这条命令依赖本机装了配方，没有配方就完全不可用。** `frago desktop` 随 frago 包分发，但它驱动的舞台不随包走——真正干活的 `agent_os` 与 `agent_os_ui` 两个配方住在 `~/.frago/recipes/` 下。所以换一台机器，命令在、能力可能不在。**frago 包不带任何配方**，`frago init` 装不来它们，它会正常跑完并报告成功，然后这条命令依然找不到舞台。这两份也不在社区仓库里，得从你已有的那台机器同步过来。
