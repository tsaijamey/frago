...
所以 `/run` 一律 404，`config.json` 里 `actions` 恒为空——配方在 `page_actions` 里开了什么都不算数。
...
否则 `frago recipe validate` 直接报错——它会把每个人的产出都写进配方写死的那同一个目录。
...
**NEVER 把会花钱、会以你的名义对外做事的 mode 写进 `page_actions`。**
...
- 不要指望 `page_actions` 是沙箱。配方以跑 frago 的那个系统用户的身份和权限跑，隔离只保证产出落点不串人。
