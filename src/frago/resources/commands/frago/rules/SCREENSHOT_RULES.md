...
| 场景 | ✅ 必须这样做 | ⛔ 禁止这样做 |
|-----|-------------|-------------|
| 获取推文/帖子内容 | `frago get-content` 或配方 | 截图后让 AI "阅读" |
| 获取评论列表 | `frago get-content` 或配方 | 截图整个评论区 |
| 获取页面任何文字 | `frago get-content` 或 `exec-js` | 截图后让 AI "阅读" |
| 获取 DOM 结构 | `frago exec-js` 提取 | 截图整个页面 |
...
在使用 `frago screenshot` 前必须回答：

> "我是否打算用这张截图来阅读页面内容？"
> - 如果是 → **禁止截图**，改用 `get-content`
> - 如果否 → 可以截图
