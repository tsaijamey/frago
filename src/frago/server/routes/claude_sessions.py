"""Claude Code sessions API endpoints (read-only).

Powers the session pages' Claude-Code-specific reads: list raw Claude Code
sessions from ``~/.claude/projects`` (classified human / maybe / agent), read a
single session's message stream, and aggregate the token calendar.

发送那条路**不在这里**。它从前叫 ``POST /api/claude-sessions/{sid}/send``，只驱动
claude 一家；工作台清单里躺着三家的会话，所以它搬去了 ``/api/workbench/sessions/
{sid}/send``（见 ``routes/workbench.py``）并补齐了 opencode 与 codex。名字留在这里
会说假话：对着 codex 的一行说话，请求却发去一个叫 claude-sessions 的地方。
"""
