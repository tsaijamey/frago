...
install_dependency() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        if command -v brew &> /dev/null; then
            ...
            echo '{"success": false, "error": {"code": "MISSING_DEPENDENCY", "message": "Homebrew not installed"}}'
            exit 2
        fi
        ...
        if command -v uv &> /dev/null; then
            ...
            echo "{\"success\": false, \"error\": {\"code\": \"MISSING_DEPENDENCY\", \"message\": \"Cannot auto-install $pkg_name. Please install manually.\"}}"
            exit 2
        fi
    else
        echo "{\"success\": false, \"error\": {\"code\": \"UNSUPPORTED_OS\", \"message\": \"Unsupported OS: $OSTYPE\"}}"
        exit 2
    fi

    # 验证安装成功
    if ! command -v "$cmd" &> /dev/null; then
        echo "{\"success\": false, \"error\": {\"code\": \"INSTALL_FAILED\", \"message\": \"Failed to install $pkg_name\"}}"
        exit 2
    fi
...
# 解析输入参数（从命令行参数读取 JSON）
if [ $# -eq 0 ]; then
    echo '{"success": false, "error": {"code": "MISSING_PARAMS", "message": "No parameters provided"}}'
    exit 1
fi
INPUT="$1"
...
# 验证必需参数
if [ -z "$URL" ]; then
    echo '{"success": false, "error": {"code": "MISSING_PARAMS", "message": "url is required"}}'
    exit 1
fi
...
if eval $YT_DLP_CMD 2>&1 | tee "$TEMP_OUTPUT"; then
    ...
    rm -f "$TEMP_OUTPUT"
    echo '{"success": false, "error": {"code": "DOWNLOAD_FAILED", "message": "yt-dlp download failed"}}'
    exit 1
fi
