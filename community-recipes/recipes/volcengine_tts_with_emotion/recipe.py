...
RECIPE_NAME = "volcengine_tts_with_emotion"
...
def synthesize_speech(
    ...
    # 获取凭证
    app_id = secrets.get("app_id") or os.environ.get("X_APP_ID") or os.environ.get("X-APP-ID")
    access_key = secrets.get("access_token") or os.environ.get("X_ACCESS_TOKEN") or os.environ.get("X-ACCESS-TOKEN")
    voice_id = speaker_id or secrets.get("voice_id") or os.environ.get("VOICE_ID")

    if not app_id:
        return {"success": False, "error": "缺少 app_id，请检查 ~/.frago/recipes.local.json 或环境变量"}
    if not access_key:
        return {"success": False, "error": "缺少 access_token，请检查 ~/.frago/recipes.local.json 或环境变量"}
    if not voice_id:
        return {"success": False, "error": "缺少 voice_id，请通过参数、recipes.local.json 或环境变量提供"}
