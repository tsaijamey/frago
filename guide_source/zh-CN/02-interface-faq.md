...
**A**: Console是Recipe开发专用环境（自动批准所有操作），Tasks是正常任务执行环境（需要手动批准，显示执行历史）。

这是frago最容易混淆的概念。让我们用表格对比：

| 特性 | Console | Tasks |
|------|---------|-------|
| **用途** | 开发和测试Recipe | 执行正常任务 |
| **批准模式** | 自动批准（所有工具调用直接执行） | 交互式批准（需要手动确认）* |
| **会话显示** | 当前开发会话 | 所有历史会话 |
| **使用场景** | 探索网页、调试Recipe、快速实验 | 执行已知任务、查看历史记录 |
| **安全性** | ⚠️ 较低（自动执行） | ✅ 较高（手动控制） |

> *注：Tasks的交互式批准需要配合Claude Code CLI使用。在Web UI中启动的任务也会自动批准。

**使用建议**：

- **用Console开发**：当你想让AI帮你探索如何完成某个任务时（比如"帮我研究如何从这个网页提取数据"）
- **用Tasks查看**：当你想查看之前执行过的任务历史，或者通过CLI启动任务时
...
**A**: Recipe是可重用的自动化脚本，包含完整的任务描述、参数定义和执行逻辑。可以把它理解为"保存好的菜谱"。
...
**A**: Recipe是可执行的自动化脚本（代码），Skill是指导Claude工作的方法论文档（文字）。新手可以先忽略Skills。

**简单类比**：

- **Recipe**：做菜的步骤脚本，机器可以直接执行
