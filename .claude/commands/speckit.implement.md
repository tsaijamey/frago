...
1. 从仓库根目录运行 `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于参数中的单引号如 "I'm Groot",使用转义语法:例如 'I'\''m Groot'(或者如果可能使用双引号:"I'm Groot")。
...
2. **检查清单状态**(如果 FEATURE_DIR/checklists/ 存在):
   ...
   - **如果任何清单未完成**:
     - 显示带有未完成项目数的表格
     - **停止**并询问:"某些清单未完成。您仍要继续实施吗?(是/否)"
     - 等待用户响应后再继续
     - 如果用户说"否"或"等待"或"停止",则停止执行
     - 如果用户说"是"或"继续"或"前进",则继续步骤 3
