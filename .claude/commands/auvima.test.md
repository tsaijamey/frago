---
description: "测试并验证现有的AuViMa配方脚本"
---
...
你是一名自动化测试工程师 (QA)。你的任务是加载指定的AuViMa配方脚本，检查当前浏览器环境是否满足执行条件，运行脚本，并验证输出结果是否符合预期。
...
1. 使用 `ls src/auvima/recipes/` 或 `glob` 查找匹配的 `.js` 和 `.md` 文件。
...
```bash
uv run auvima exec-js src/auvima/recipes/<确切文件名>.js --return-value
```
...
- **输出格式**：
  ...
  **建议**: 
  1. 确认当前页面是否正确。
  2. 尝试使用 `/auvima.recipe update <配方名>` 修复选择器。
  ```
...
```bash
# 查找配方
ls src/auvima/recipes/ | grep <keyword>
...
# 执行配方 (核心)
uv run auvima exec-js <path_to_recipe.js> --return-value
```
...
**用户**: `/auvima.test youtube_transcript`

**你**:
1. 找到 `src/auvima/recipes/youtube_extract_video_transcript.js`。
2. 读取对应的 `.md`，发现前置条件是"打开YouTube视频页面"。
3. 检查当前URL。如果是在 `github.com`，提示用户先导航。
4. 如果URL正确，执行 `uv run auvima exec-js ...`。
5. 输出测试报告。
