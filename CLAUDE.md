...
从004-recipe-architecture-refactor开始，AuViMa采用元数据驱动的Recipe架构，支持多运行时和Workflow编排。

#### 核心特性

- **三级优先级**: Project (.auvima/recipes/) > User (~/.auvima/recipes/) > Example (examples/)
...
**列出所有 Recipe**:
```bash
uv run auvima recipe list                    # 表格格式
uv run auvima recipe list --format json      # JSON格式（AI使用）
uv run auvima recipe list --source project   # 仅项目级
```

**查看 Recipe 详情**:
```bash
uv run auvima recipe info <recipe_name>
uv run auvima recipe info <name> --format json  # JSON格式
```
...
**使用 /auvima.recipe 命令创建新Recipe**:
```
/auvima.recipe create "批量提取Upwork职位"
```

**使用 /auvima.test 命令测试Recipe**:
```
/auvima.test youtube_extract_video_transcript
```
